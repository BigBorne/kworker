...
# ── Конфигурация ─────────────────────────────────────────────
REPO_URL="https://github.com/BigBorne/kworker.git"
BOT_DIR="$HOME/kworker"
...
# ── Интерактивная настройка .env.exemple ─────────────────────────────
configure_env() {
...
start_bot() {
    ...
    # Нет .env.exemple → нельзя запустить
...
main() {
    ...
    if [[ ! -d "$BOT_DIR/.git" ]] || [[ ! -f "$VENV_DIR/bin/python" ]]; then
        ...
        # Записать .env.exemple после клонирования репозитория (если ещё нет)
        local env_file="$BOT_DIR/.env"
