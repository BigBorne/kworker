    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS order_history (
                ...
                kwork_published_at TEXT NOT NULL DEFAULT '',
                offers_count  INTEGER NOT NULL DEFAULT 0,
                time_left     TEXT    NOT NULL DEFAULT '',
                buyer_username       TEXT    NOT NULL DEFAULT '',
                buyer_total_projects INTEGER NOT NULL DEFAULT 0,
                buyer_hired_percent  INTEGER NOT NULL DEFAULT 0,
        ...
        for stmt in [
            ...
            "ALTER TABLE order_history ADD COLUMN kwork_published_at TEXT DEFAULT ''",
            "ALTER TABLE order_history ADD COLUMN offers_count INTEGER DEFAULT 0",
            "ALTER TABLE order_history ADD COLUMN time_left TEXT DEFAULT ''",
            "ALTER TABLE order_history ADD COLUMN buyer_username TEXT DEFAULT ''",
            "ALTER TABLE order_history ADD COLUMN buyer_total_projects INTEGER DEFAULT 0",
            "ALTER TABLE order_history ADD COLUMN buyer_hired_percent INTEGER DEFAULT 0",
        ]:
            try:
        ...
        cur = await db.execute(
            ...
            INSERT OR IGNORE INTO order_history
                (order_id, category_id, title, description, budget, price_min,
                 category_name, kwork_published_at, offers_count, time_left,
                 buyer_username, buyer_total_projects, buyer_hired_percent)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                order.get("kwork_published_at", "") or "",
                int(order.get("offers_count", 0) or 0),
                order.get("time_left", "") or "",
                order.get("buyer_username", "") or "",
                int(order.get("buyer_total_projects", 0) or 0),
                int(order.get("buyer_hired_percent", 0) or 0),
            ),
        )
        await db.commit()
        return cur.rowcount > 0
